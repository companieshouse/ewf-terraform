# ------------------------------------------------------------------------------
# EWF BEP ALB Security Group and rules
# ------------------------------------------------------------------------------
module "ewf_bep_internal_alb_security_group" {
  source  = "terraform-aws-modules/security-group/aws"
  version = "~> 3.0"

  name        = "sgr-${var.application}-bep-internal-alb-001"
  description = "Security group for the ${var.application} BEP"
...
#--------------------------------------------
# EWF BEP Internal ALB
#--------------------------------------------
module "ewf_bep_internal_alb" {
  source  = "terraform-aws-modules/alb/aws"
  version = "~> 5.0"

  name                       = "alb-${var.application}-bep-internal-001"
  vpc_id                     = data.aws_vpc.vpc.id
  internal                   = true
  load_balancer_type         = "application"
  enable_deletion_protection = true

  security_groups = [module.ewf_bep_internal_alb_security_group.this_security_group_id]
  ...
  target_groups = [
    {
      name                 = "tg-${var.application}-bep-internal-001"
      backend_protocol     = "HTTP"
      backend_port         = var.bep_service_port
      target_type          = "instance"
      deregistration_delay = 10
      health_check = {
        enabled             = true
        interval            = 30
        path                = var.bep_health_check_path
        port                = var.bep_service_port
        healthy_threshold   = 3
...
#--------------------------------------------
module "ewf_bep_internal_alb_alarms" {
  source = "git@github.com:companieshouse/terraform-modules//aws/alb-cloudwatch-alarms?ref=tags/1.0.116"

  alb_arn_suffix            = module.ewf_bep_internal_alb.this_lb_arn_suffix
  target_group_arn_suffixes = module.ewf_bep_internal_alb.target_group_arn_suffixes

  prefix                    = "ewf-internal-bep-"
